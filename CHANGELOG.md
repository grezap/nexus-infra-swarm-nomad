...
- **`role-overlay-consul-acl.tf` (v6): Stage 4 self-validates each KV agent token against the live Consul** (`consul acl token read -self`) before reusing it; if it doesn't resolve (cold rebuild = fresh Consul, persisted KV), re-create the per-host policy + token. Previously Stage 4 reused on KV presence alone, so a cold rebuild left the fresh Consul with zero agent policies/tokens (smoke 0.E.2.3 "6 agent-* policies / ≥7 tokens" failed). Mirrors Stage 3's mgmt-token validation.
- **`role-overlay-nomad-acl.tf` (v3): Stage 4 self-validates each KV agent token against the live Nomad** (`nomad acl token self`) before reusing it; same fix as consul-acl v6 (smoke 0.E.3.2 "1 mgmt + 6 agent" failed otherwise).
- **`scripts/smoke-0.E.4.ps1`: the admin-password check now asserts the PLAINTEXT** (`^[A-Za-z0-9]{12,}$`) instead of a bcrypt shape — it had encoded the same wrong assumption as the v2 render overlay.

### Fixed (2026-06-19) — Portainer admin authentication

- **`role-overlay-portainer-admin-render.tf` (v3): render the PLAINTEXT admin password, not the bcrypt hash.** Portainer's `--admin-password-file` reads the file content as the **plaintext** password and bcrypts it internally — only the `--admin-password` CLI flag takes a pre-computed bcrypt. v2 wrote the bcrypt hash to `/etc/portainer/admin-password.txt`, so Portainer bcrypted the bcrypt-string and the real admin password became the `$2b$…` string; the Vault KV `nexus/portainer/admin-bcrypt.plaintext` (and the bcrypt, and `admin`) all 401'd. Live-caught during the nexus-cli v0.8.2 SwarmAdapter close-out (the `topology swarm` `/api/endpoints` enrichment couldn't authenticate). Also fixed the render-wait probe (was matching a bcrypt shape) + the misleading comments in the render + seed overlays.
- **`role-overlay-portainer-stack.tf` (v2): post-deploy reconcile — self-heal a stale NFS boltdb + register the local Swarm environment.** Portainer's boltdb lives on the gateway NFS share (`/var/lib/portainer-data`), so it **persists across a swarm destroy/apply**; `--admin-password-file` is only consumed at first-init, so an admin set from an older KV seed (e.g. before the v0.8.1 Vault greenfield re-seed) survives and desyncs from the current KV plaintext. The deploy now probes admin auth and, if it fails, re-initializes from the (plaintext) file (scale 0 → backup+wipe `portainer.db` → scale 1); then idempotently registers the `local-swarm` agent environment (`tcp://tasks.agent:9001`, TLS-skip) so a fresh Portainer is fully usable with zero manual UI steps. Live-remediated on the running cluster 2026-06-19 (admin auth → 200, environment registered); **the automated overlay path is cold-rebuild-proven 2026-06-20** — a destroy→apply with a deliberately-restored desynced boltdb auto-healed (`[reconcile] admin auth failed (422) … re-initializing … post-reset admin auth: 200 … endpoint create http 200`).
