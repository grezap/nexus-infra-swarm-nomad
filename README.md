...
>
> **➜ Want to rebuild the orchestration tier from zero?** [`docs/handbook.md`](./docs/handbook.md) is the operator canon — §0 prerequisites (foundation + security tiers must be alive), §1 full Phase 0.E walkthrough (a single `swarm.ps1 apply` brings up swarm + Consul + Nomad + Portainer + the 0.E.4e fixes — every sub-phase enumerated in §1.2), §1.4 selective-ops `-Vars` examples (e.g. "managers only"), §3.6 cold-rebuild canon. Cross-tier index: [`nexus-platform-plan/docs/setup-guides.md`](https://github.com/grezap/nexus-platform-plan/blob/main/docs/setup-guides.md).
>
...
| 0.E.4 | Portainer CE clustered Swarm service | Portainer reachable + 3 replicas |
