...
| After a **>7-day** power-off, all 3 Consul servers stuck `activating`; the cluster never forms — but `nexus status swarm` looks fine (it reads docker, not consul) | Consul refuses to rejoin a raft that has been offline past `server_rejoin_age_max` (default **168h/7d**); this lab is normally powered off between sessions | **Baked fix (2026-07-06):** the consul-server template now sets `server_rejoin_age_max = "8760h"` (`consul-server.hcl.j2`), so the servers rejoin their **own intact raft** (ACL state preserved). On an older node, add that line to `/etc/consul.d/consul.hcl` + restart consul. **Do NOT wipe the data dir** — it orphans the ACL bootstrap. See `feedback_swarm_consul_rejoin_age_freeze`. |
